...
public class JwtUtil {
    //Jwt秘钥
    private static final String key = "shixunxiangmu";

    //根据用户信息创建Jwt令牌
    public static String createJwt(UserDetailsImpl user){
    ...
    public static UserDetails resolveJwt(String token){
        ...
        try {
            ...
            if(new Date().after(claims.get("exp").asDate())) //如果是过期令牌则返回null
                ...
                return new UserDetailsImpl(new User().setUsername(claims.get("username").asString()),
                        Arrays.stream(claims.get("authorities").asArray(String.class)).toList());
//                return User
