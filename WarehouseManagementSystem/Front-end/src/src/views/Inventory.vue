  ...
  <div class="modal-body">
    <form @submit.prevent="handleStockIn">
      ...
      <div class="form-group">
        ...
        <input type="number" class="form-control" id="StockQuantity" v-model="newPart.quantity" placeholder="请输入数量" required>
      </div>
    </form>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      showStockIn: false,
      showStockOut: false,
      newPart: { productName: '', specification: '', quantity: 0 },
  ...
  methods: {
    ...
    async fetchInventory() {
      try {
        ...
        // console.log(打印库存this.inventory.StockQuantity);
      } catch (error) {
    ...
    async handleStockIn() {
      try {
        ...
        console.log(打印后端发回入库反馈response.data);
        if (response.data.success) {
          this.fetchInventory();
          this.hideModals();
          alert('入库成功！');
